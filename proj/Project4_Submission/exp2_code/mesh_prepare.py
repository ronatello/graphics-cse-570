...
def extract_features(mesh):
    features = []
    edge_points = get_edge_points(mesh)
    """To consider all five edge centroids"""
    for i in range(edge_points.shape[0]):
      centroids = []
      edge_vertex_1 = np.array(mesh.vs[edge_points[i,0]])
      edge_vertex_2 = np.array(mesh.vs[edge_points[i,1]])
      edge_vertex_3 = np.array(mesh.vs[edge_points[i,2]])
      edge_vertex_4 = np.array(mesh.vs[edge_points[i,3]])

      centroid_vertex_1 = np.array(edge_vertex_1 + edge_vertex_2) / 2 
      centroids.append(np.average(centroid_vertex_1))
      # centroids.append(np.average(centroid_vertex_1))
      centroid_vertex_2 = np.array(edge_vertex_1 + edge_vertex_3) / 2
      centroids.append(np.average(centroid_vertex_2))
      # centroids.append(np.average(centroid_vertex_2))
      centroid_vertex_3 = np.array(edge_vertex_1 + edge_vertex_4) / 2
      centroids.append(np.average(centroid_vertex_3))
      # centroids.append(np.average(centroid_vertex_3))
      centroid_vertex_4 = np.array(edge_vertex_2 + edge_vertex_4) / 2
      centroids.append(np.average(centroid_vertex_4))
      # centroids.append(np.average(centroid_vertex_4))
      centroid_vertex_5 = np.array(edge_vertex_2 + edge_vertex_3) / 2
      centroids.append(np.average(centroid_vertex_5))
      # centroids.append(np.average(centroid_vertex_5))
      centroids = np.array(centroids)
      features.append(centroids)
    features = np.array(features)
    # print(features.shape)
    return features.T

    """To consider only one edge centroid"""
    # for i in range(edge_points.shape[0]):
    #   edge_vertex_1 = np.array(mesh.vs[edge_points[i,0]])
    #   edge_vertex_2 = np.array(mesh.vs[edge_points[i,1]])
    #   centroid_vertex = np.array(edge_vertex_1 + edge_vertex_2) / 2

    #   features.append(centroid_vertex)
    # features = np.array(features)
    # return features.T
